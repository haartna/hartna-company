...
export default function SubcategoryPage({ params }: { params: { subcategory: string } }) {
  ...
  const filteredProducts = products.filter(
    (p) => p.category === "others-machines" && p.subcategory === params.subcategory,
  )
      ...
      <main className="flex-1">
        ...
        <section className="py-12 bg-muted/50">
          <div className="container mx-auto px-4">
            <div className="mb-4">
              <Link href="/products/others-machines" className="text-sm text-muted-foreground hover:text-foreground">
                ← {t("backToAllMachines")}
              </Link>
            </div>
